...
def check_special_questions(countries):
    if len(countries) == 2:
        if check_special_country(countries, ["ECU", "SRB"]): return 1
        if check_special_country(countries, ["AZE", "UZB"]): return 2
        if check_special_country(countries, ["KWT", "ARE"]): return 3
        if check_special_country(countries, ["MLI", "GIN"]): return 4
        if check_special_country(countries, ["TCD", "ROU"]): return 5
        if check_special_country(countries, ["AUS", "NZL"]): return 6
        if check_special_country(countries, ["RUS", "NLD"]): return 7
        if check_special_country(countries, ["WSM", "PRK"]): return 2
        if check_special_country(countries, ["ISL", "NOR"]): return 9
        if check_special_country(countries, ["LBR", "USA"]): return 2
        if check_special_country(countries, ["TUN", "TUR"]): return 11
        if check_special_country(countries, ["DNK", "GEO"]): return 14
        #if check_special_country(countries, ["PRI", "CHL"]): return 15
        if check_special_country(countries, ["GRL", "IDN"]): return 16

    elif len(countries) == 3:
        if check_special_country(countries, ["PRI", "CHL", "PAN"]): return 2
        #if check_special_country(countries, ["CHE", "DNK", "GEO"]): return 14
        #if check_special_country(countries, ["GRL", "IDN", "MCO"]): return 13
    return 0
